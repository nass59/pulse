...
- **Build-state honesty in diagrams.** `<SystemTopology>` encodes `live` (running today) vs `planned` (designed, not built) nodes, and keeps edges dashed until a service exists to drive them. Each epic flips a node — and its edges — from planned to live. Don't draw a node as live before its service ships. Its per-node contract panel lists events sourced from `EVENT_TOPICS` in `@pulse/schemas/topics` (wired via `transpilePackages`), not a hand-copied list — so the diagram can only show contracts the codebase actually defines, and new topics appear automatically. Keep it that way; don't reintroduce a literal event list that can drift.
...
A second polish pass tuned the _reading experience_ without changing what the site is. These are voice/surface conventions, still downstream of [ADR-0007](../../docs/adr/0007-docs-site-as-study-artifact.md) (the site stays a derived study artifact that tracks the build — positioning unchanged, no ADR amendment). Resolved in a `grill-with-docs` run.
...
- **Motivating example lives on the homepage.** One strong animated narrative — the livestream firehose (a creator goes live, viewers flood the Channel, chat becomes a firehose) — anchors the homepage. It uses canonical `CONTEXT.md` terms (Stream, Channel, control/media plane); "firehose" is an analogy, deliberately _not_ a glossary term. Concept pages get a single one-line "why you'd hit this" hook, not a repeated full scenario.
- **Build-state honesty extends to the homepage roadmap.** The homepage roadmap (`components/home/roadmap/phase-drawers.tsx`, an accordion over the five phases) is the public, derived read of `.scratch/ROADMAP.md` — its data lives in `lib/roadmap.ts`. Phase status (`shipped`/`current`/`planned`) is hand-set from the real issue tracker, never invented progress; the `current` phase is the only "you are here". Concept items with an `href` are `live` and link out, the rest are `coming` (dashed, no link) — the same honesty rule as `<SystemTopology>`, so don't link a concept page before it ships. Effort is weekend-scale, never calendar dates, and nothing links into local-only `.scratch/`. Two alternative visualisations of the same `lib/roadmap.ts` live alongside it (`signal-path.tsx`, `build-pipeline.tsx`) and are showcased on the unlinked `/roadmap-preview` page; the flat live/coming catalogue now lives on each pillar overview via `pillar-overview.tsx`.
...
A third pass fixed how content _renders_ and made every content page share one skeleton. These are downstream of [ADR-0010](../../docs/adr/0010-mdx-rendering-pipeline.md) (the MDX pipeline) and ADR-0009 (the design system) — not new decisions. Resolved in a `grill-with-docs` run.

- **Tables are GFM markdown, never hand-rolled JSX.** `remark-gfm` is wired into `createMDX`, so `| … |` tables parse and flow through the styled `Table`/`Th`/`Td` components mapped in `mdx-components.tsx`. Author tables as markdown — don't assemble `<Table><Tr><Td>` by hand. Enrich _inside_ cells (inline `code` for images/ports, links for cross-references); the wrapper owns the rounded, bordered, horizontally-scrollable chrome.
...
- **No literal backticks in component data.** Strings in `lib/*.ts` and `components/**` data (card `blurb`s, titles) render _verbatim_ — a backtick in the string shows as a backtick on screen. Use a real `<code>` element in JSX, or plain prose. Backtick-as-markdown only works inside `.mdx` body text.
...
## Information architecture — three pillars + The Build (ADR-0021)

The site is organised into **three technology pillars** — `/kafka`, `/go`, `/kotlin` — plus a cross-cutting `/build` hub, replacing the old flat `Learn`/`Concepts`/`Architecture`/`Journey`/`Go` tiers. Downstream of [ADR-0021](../../docs/adr/0021-docs-site-technology-pillars.md); follow it when adding pages.

- **Each pillar has one shape: overview + `path/` + `concepts/`.** The overview (`app/<pillar>/page.mdx`) renders `<PillarOverview pillar="…" />` — the single map showing both halves. `path/<slug>` holds the ordered, build-state-**independent** lessons (teach the technology from zero; every step `live`). `concepts/<slug>` holds the build-state-**gated** reference pages (a lit card means Pulse runs that idea). Put a fundamentals page in `path/`, a Pulse-specific page in `concepts/` — never blur the two.
- **The Build (`/build`) holds the cross-cutting, gated system view:** `architecture/` (topology, infra, plus the infra concepts `healthchecks` and `named-volumes`) and `journey/` (per-epic recaps). Anything about the whole system rather than one technology goes here.
- **Pillar data lives in `lib/pillars.ts`** (overview specs + the `PILLAR_PATHS` registry the sticky `<PathRail>` reads). Concept shelves are the per-pillar arrays in `lib/concepts.ts` (`KAFKA_CONCEPTS`, `GO_CONCEPTS`, `BUILD_CONCEPTS`, `KOTLIN_COMING_CONCEPTS`). Add a concept by appending there with an `href` once its page ships — same build-state honesty as before.
- **Per-technology accent (ADR-0020, extended).** Kafka = electric-yellow, Go = `go-blue`, Kotlin = `kotlin-purple` (`.ds-mark-kotlin`, `shadow-glow-kotlin-*`). Accent is semantic: it marks which technology a surface is about. The Build uses the Kafka/brand yellow — it's Pulse's own surface, not a guest technology.
- **The sticky `<PathRail>`** (in the root layout) shows "you are here" **only** on `path/` pages, driven by `PILLAR_PATHS`. `concepts/`, `/build`, and the homepage are not sequences and get plain `<PageNav>` only.
