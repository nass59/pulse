...
- **librdkafka** (`confluent-kafka-go`, the `chat` client) — defaults to
  `consistent_random`, a **CRC32** hash, _not_ murmur2. ✗ Must be explicitly set
  to `partitioner=murmur2_random`. (`chat`'s producer does this as of issue 03;
  verified live — a channel's messages co-partition.)
...
- The Go `chat` producer sets `partitioner=murmur2_random` (issue 03, done).
