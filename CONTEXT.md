...
- **Source of schemas:** monorepo, in `packages/schemas/`. Each service runs Avro codegen from the shared `.avsc` files at build time. The registry _enforces_ compatibility (the publish script pins every subject to `BACKWARD` and the registry rejects a break — today run by hand; wiring it into CI is still to-do); the `.avsc` files in the repo are the human-readable SoT.
