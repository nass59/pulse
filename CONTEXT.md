...
A single live WebSocket connection from a viewer to `chat`. It is the unit of presence: `ViewerJoined` is emitted when a session opens and passes the liveness gate, `ViewerLeft` when it closes (for any reason), and "concurrent viewers" is the count of currently-open sessions — which `analytics` approximates via windowing. The count is **approximate by design**: a `ViewerLeft` can be lost (a node that dies without flushing leaves a join with no matching leave), so join and leave are not a balanced ledger — windowing tolerates this by ageing an unmatched join out of the count rather than letting it inflate forever. One account may hold several simultaneous sessions (e.g. two browser tabs), and each counts; presence is session-grained, not account-grained. (In the MVP the `userId` is hardcoded, so every session shares one account — the count still reflects sessions, which is the point.) _Avoid:_ deduplicating presence by `userId`; treating presence as a balanced ledger — reconciling or synthesising leaves for dangling sessions to make the count exact fights the windowed, approximate design that pushes crash-consistency downstream on purpose; or treating "viewer count" as "distinct accounts watching."
